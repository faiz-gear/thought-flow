...
import ReactFlow, { Node, Edge, Controls, Background, NodeProps, Handle, Position, MiniMap } from 'reactflow'
    ...
    >
      <CardContent className="p-4 flex flex-col gap-2">
        <div className="flex items-center justify-between">
          ...
          <div className="text-xs text-muted-foreground rounded-md bg-muted px-2 py-1">Level {data.depth}</div>
        </div>
        <div className="flex flex-col gap-1.5">
          <Progress value={data.ratio * 100} className="h-2" />
          <div className="text-xs text-muted-foreground text-right">用时比例 {Math.round(data.ratio * 100)}%</div>
        </div>
        {!hasChildren && data.label.length > 1 && (
          <Button size="sm" variant="outline" onClick={handleDecompose} disabled={isDecomposing}>
            {isDecomposing ? (
              <>
...
const convertFlowDataToNodesAndEdges = (
  ...
  const processNode = (nodeData: FlowData, parentId: string | null, x: number, y: number) => {
    const isHighlighted = highlightedNodes === null || highlightedNodes.has(nodeData.id)
    const node: Node = {
      id: nodeData.id,
      type: 'custom',
      position: { x, y },
    ...
    if (nodeData.children.length > 0) {
      const totalWidth = nodeData.children.length * NODE_WIDTH + (nodeData.children.length - 1) * NODE_SPACING
...
export function FlowChart() {
  ...
  const getAncestorIds = useCallback(
    (nodeId: string, data: FlowData | null): { ancestors: string[]; siblings: string[] } => {
      ...
      const findAncestorsAndSiblings = (
        ...
        if (currentNode.id === targetId) {
          const siblings = parent
            ? parent.children.filter((sibling) => sibling.id !== targetId).map((sibling) => sibling.id)
            : []
        ...
        for (const child of currentNode.children) {
          const result = findAncestorsAndSiblings(child, targetId, [...path, currentNode.id], currentNode)
          if (result) {
            return result
          }
        }
  ...
  // 获取节点的所有子孙节点ID
  const getDescendantIds = useCallback((nodeId: string, data: FlowData | null): string[] => {
    if (!data) return []
  ...
  const handleNodeMouseEnter = useCallback(
    (nodeId: string) => {
      ...
      const highlighted = new Set([nodeId, ...ancestors, ...siblings, ...descendants])
  ...
  // 从边获取相关节点ID
  const getNodesFromEdge = useCallback((edgeId: string): { sourceId: string; targetId: string } => {
  ...
  useEffect(() => {
    if (flowData) {
      const { nodes: newNodes, edges: newEdges } = convertFlowDataToNodesAndEdges(
        flowData,
        null,
        0,
        0,
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      toast({
        variant: 'destructive',
        title: '错误',
        description: error instanceof Error ? error.message : '生成流程图时出错'
      })
    } finally {
      setIsLoading(false)
    }
  }, [input, resetFlow, setFlowData, settings, toast])
